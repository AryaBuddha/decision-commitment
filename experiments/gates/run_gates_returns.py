"""Gates 1-5 for environment 7 (returns, WP2 Phase 5).
Run: python experiments/gates/run_gates_returns.py"""
...
from cus.envs import returns as sp                  # noqa: E402
from cus.envs.returns import ReturnsEnv, generate, gold  # noqa: E402

GATE_CONFIG = {
    "experiment": "env_returns_gates",
    "env_seed": 20260821, "n_demo": 6000, "expert_noise": 0.07,
    "max_depth": 6, "min_samples_leaf": 50,
    "tilt_features": ["serial_rate", "desc_vagueness"],
    "primary_tilt": "serial_rate",
...
def main():
    ...
    freeze = json.loads((ROOT / "registrations" / "env_returns.json").read_text())
    if freeze.get("gate_config_hash") != h:
        raise SystemExit(f"Gate config hash {h} != frozen {freeze.get('gate_config_hash')}")
    print(f"[freeze] returns: gate config {h} matches")
    cfg = GATE_CONFIG
    env = ReturnsEnv.induce(cfg["env_seed"], cfg["n_demo"], cfg["expert_noise"],
    ...
    print(f"[gates] returns: {'ALL PASS' if report['all_pass'] else 'FAILED'}")
    out = ROOT / "artifacts" / f"gates_returns_{h}"
